...
firewallRule=$(gcloud compute firewall-rules list --format=value\(name\) --filter=name=lego-svc-rule)
echo "You are ready to verify '$firewallRule' for network traffic."

if [ "$firewallRule" = "lego-svc-rule" ]; then
	echo "Rule found. You are ready to update this rule to allow traffic from port $nodePort."
	$(gcloud compute firewall-rules update lego-svc-rule --allow=tcp:$nodePort)
else
	echo "Rule not found! You are ready to create '$firewallRule' to allow traffic from port $nodePort."
	$(gcloud compute firewall-rules create lego-svc-rule --allow=tcp:$nodePort)
fi

# Obtain the external IP
#   `-> for some reason jsonpath is required in double-quotes instead of single-quote o/w it fails!
externalIp=$(kubectl get node -o jsonpath="{$..addresses[?(@.type=='ExternalIP')].address}")
